...
def analyzer_contours_with_dt(prefix, dts, levels = 100, cmap = 'jet', fp = Path.cwd() / "solution"):

    file_list = []
    for dt in dts:
        file_list.append(prefix + f"_dt{dt:.1e}.h5")
    ...
    #####################################################
    # CONTOUR PLOT W
    vmin = min(np.nanmin(d[-1]) for d in fcontour_data)
    vmax = max(np.nanmax(d[-1]) for d in fcontour_data)
...
def analyzer_contours_with_Ne(prefix, dt, levels = 100, cmap = 'jet', fp = Path.cwd() / "solution"):
    ...
    ax3[0].set_ylabel("t", rotation = 0, labelpad = 10)
